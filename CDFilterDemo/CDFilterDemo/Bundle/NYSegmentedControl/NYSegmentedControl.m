...
- (void)initialize {
    ...
    _segmentedControlSize = CGSizeZero;
...
- (CGSize)sizeThatFits:(CGSize)size {
    ...
    if (CGSizeEqualToSize(_segmentedControlSize, CGSizeZero)) {
        return CGSizeMake(maxSegmentWidth * [self.segments count], 30.0f);
    }
    else {
        return _segmentedControlSize;
    }
}
            ...
            if (CGRectContainsPoint(segment.frame, self.selectedSegmentIndicator.center)) {
                ...
                if (index != self.selectedSegmentIndex) {
                    ...
                    if ([self.delegate respondsToSelector:@selector(segmentedControl:didSelectedAtIndex:)]) {
                        [self.delegate segmentedControl:weakSelf didSelectedAtIndex:index];
                    }
                }
            }
        }];
